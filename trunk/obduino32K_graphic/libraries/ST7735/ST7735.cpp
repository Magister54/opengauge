...
void ST7735::initB(void) {
  ...
  if (_sclk) {
    ...
    SPI.begin();
    SPI.setDataMode(SPI_MODE0); //SPI_MODE3
  }
...
void ST7735::initR(void) {
  ...
  writecommand(ST7735_MADCTL);  // memory access control (directions)
  writedata(0xA8); //A8//68//B8 // row address/col address, bottom to top refresh
  madctl = 0xA8;
...
void ST7735::drawString(uint8_t x, uint8_t y, char *c, uint16_t color, uint8_t size) 
{
  while (c[0] != 0) 
  {
...
// draw a character
void ST7735::drawChar(uint8_t x, uint8_t y, char c, uint16_t color, uint8_t size) 
{
...
uint16_t ST7735::modifyColor(uint16_t color, uint8_t size)
{
  return (((((color & 0xF800) >> 11) << 4) / size) << 11) | 
         (((((color & 0x07E0) >>  5) << 4) / size) <<  5) | 
         (((((color & 0x001F) >>  0) << 4) / size) <<  0); 
}
//--------------------------------------------------------------------------------

// redraw a character
byte ST7735::redrawChar(uint8_t x, uint8_t y, char c, uint16_t color, uint16_t background, uint8_t size) 
{
	#define CharWidth 6
	#define CharHeight 8
	
	uint8_t columns[CharWidth];
	uint16_t draw_color;

	uint8_t xm = x + ((size * CharWidth)  >> 1) - 1;
	uint8_t ym = y + ((size * CharHeight) >> 1) - 1;

	if ((xm >= width) || (ym >= height))
		return 0;

	for (uint8_t i = 0; i < CharWidth-1; i++)
		columns[i] = pgm_read_byte(font + c*(CharWidth-1) + i);
		
	columns[CharWidth-1] = 0;
		
	setAddrWindow(x, y, xm, ym);
  ...
  uint8_t shift = 0x01;
	...
	{ 
	  for (ys = 0; ys < (size >> 1); ys++) 
			for (x = 0; x < CharWidth; x++) 
			{
				draw_color = (columns[x] & shift) ? color : background;
					//modifyColor(background,4);;
					
				for (xs = 0; xs < (size >> 1); xs++) 
				{
					spiwrite(draw_color >> 8);    
					spiwrite(draw_color);    
				}
			}
...
void ST7735::drawCircle(uint8_t x0, uint8_t y0, uint8_t r, uint16_t color) {
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x = 0;
