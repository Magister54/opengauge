...
// redraw a character
byte ST7735::redrawChar(uint8_t x, uint8_t y, char c, 
		      uint16_t color, uint16_t background, uint8_t size) 
{
	#define CharWidth 6
	#define CharHeight 8
	...
	uint8_t xm = x+size*CharWidth -1;
	uint8_t ym = y+size*CharHeight-1;

	if ((xm >= width) || (ym >= height)) // may be >=?
		return 0;
