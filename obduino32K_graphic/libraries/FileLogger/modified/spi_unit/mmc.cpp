...
static uint32_t spiTransferLong(const uint32_t data) {
  ...
  long2char.c[3] = MiniSpi.transfer(long2char.c[3]);
  long2char.c[2] = MiniSpi.transfer(long2char.c[2]);
  long2char.c[1] = MiniSpi.transfer(long2char.c[1]);
  long2char.c[0] = MiniSpi.transfer(long2char.c[0]);
...
static char sdResponse(byte expected) {
  ...
  while ((MiniSpi.transfer(0xFF) != expected) && count )
    count--;
...
static char sdWaitWriteFinish(void) {
  ...
  while ((MiniSpi.transfer(0xFF) == 0) && count )
    count--;
...
const byte  deselect) {
  ...
  while (errorcount < CONFIG_SD_AUTO_RETRIES) {
    ...
    // Transfer command
    MiniSpi.transfer(0x40+command);
    spiTransferLong(parameter);
    MiniSpi.transfer(crc);

    // Wait for a valid response
    counter = 0;
    do {
      i = MiniSpi.transfer(0xff);
      counter++;
